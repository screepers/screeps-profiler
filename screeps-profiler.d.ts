 ...
 */
export interface ScreepsGameProfiler {
  /**
   ...
   * @param filter - parameter will limit the scope of the profiler to a specific function name
   */
   ...
   * @param filter - parameter will limit the scope of the profiler to a specific function name
   */
  stream(ticks: number, filter?: string): void;
   ...
   * @param filter - parameter will limit the scope of the profiler to a specific function name
   */
  email(ticks: number, filter?: string): void;
  ...
  /**
   * This will run for the given number of ticks, and will cause a file to be downloaded on the profiling
   * ends.
   *
   * @param ticks - controls how long the profiler should run before stopping
   * @param filter - parameter will limit the scope of the profiler to a specific function name
   ...
   *
   * @param lineCount the number of lines to output; 20 by default
   */
  output(lineCount: number): string;
}

export interface ScreepsProfilerStatic {
  /**
  ...
  /**
   * Outputs the current profile data as a string
   */
  output(passedOutputLengthLimit?: number): string;

  /**
   * Outputs the current profile data as a callgrind-formatted string
   */
  callgrind(): string | null;
   ...
   */
  registerClass<T extends object>(object: T, label: string): T;
   ...
   */
  registerObject<T extends object>(object: T, label: string): T;
   ...
   *
   * @param fn - The function to register for profiling
   * @param functionName - Name of the function, used as a label in output
   * @return the original function wrapped for profiling
   */
  registerFN<T extends (...args: any[]) => any>(
    fn: T,
    functionName?: string,
  ): T;
}
...
declare const profiler: ScreepsProfilerStatic;
